...
- **Never patch the binary without consent.** `patch` / `patch_asm` / `put_int` rewrite the program bytes and are off-limits during analysis — they are gated (`IDA_MCP_ALLOW_PATCH` + per-call `confirm=true`), used only on an explicit user request. Preview first with `dry_run` to see the exact byte change, get approval, then write; `revert_patch` / `list_patches` undo and audit. Injected Python (`py_eval` / `py_exec_file`) must not patch either. `rename` / `set_comments` / `set_type` are reversible annotations, not byte writes — those are fine.
