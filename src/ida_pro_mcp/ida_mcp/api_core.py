...
def func_query(
    ...
    for query in queries:
        ...
        # Keep response concise while preserving numeric size for filtering context.
...
def entity_query(
    ...
    for query in queries:
        ...
        if sort_by == "addr":
            ...
            rows.sort(key=lambda row: int(row.get(sort_by, 0) or 0), reverse=descending)
        else:
        ...
        # Remove internal helper fields before returning unless explicitly projected.
        for item in data:
            item.pop("size_int", None)
